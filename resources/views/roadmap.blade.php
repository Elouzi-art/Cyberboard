...
function renderRoadmap() {
    ...
    weeks.forEach(w => {
        const x = dayDiff(startDate, w) * CELL_W;
    ...
    projects.forEach(p => {
        ...
        const barStart = Math.max(0, dayDiff(startDate, ps));
        const barEnd   = Math.min(totalDays, dayDiff(startDate, pe));
        const barW     = Math.max(8, (barEnd - barStart) * CELL_W);
        const barX     = barStart * CELL_W;
        ...
        if (barEnd > 0 && barStart < totalDays) {
            html += `<a href="${p.url}" style="position:absolute;left:${barX}px;top:10px;width:${barW}px;height:22px;border-radius:3px;background:${p.color};display:flex;align-items:center;overflow:hidden;text-decoration:none" title="${p.title} — ${p.progress}%">`;
